...
# ── THE SCRAPE LIST IS PART OF THE RELEASE (micro-org#541) ───────────────────
#
# `targets/services.yaml` is rendered from the release manifest, so it is only
# ever as current as the last time somebody rendered it — and under compose that
# was every deploy, because `release-deploy.sh` did it. This script replaced
# `release-deploy.sh` and did not, so from the migration until 2026-09-02 the
# deployed target list was whatever the operator had last generated by hand.
#
# It was fourteen days and fourteen services out of date, and being out of date
# did not make it RED. Every stale name still resolved: the service merge keeps
# an ExternalName CNAME for each absorbed service so in-estate callers need not
# change, so Prometheus scraped one pod through twenty names, stored twenty
# copies of one exposition page under twenty `service` labels, and one failing
# route burned twenty error budgets at once — twenty-four alerts, three of them
# pages, for a defect in one module.
#
# Hence: refreshed HERE, from the release this script just applied, so the answer
# to "what is scraped" cannot outlive the answer to "what is deployed".
#
#   * mainnet only. The scrape list is rendered for one network — the namespace
#     it names is mainnet's by construction — and a testnet deploy re-rendering
#     it would point the mainnet job set at testnet, which is micro-org#398 with
#     the estates swapped.
#   * `--reload`: ConfigMaps and Secrets, no pod restart. Prometheus re-reads on
#     POST /-/reload and losing the TSDB to a restart on every deploy would be a
#     far worse trade than a stale target.
#   * a partial run (`--wave`) does not do it, because a wave is not a release.
#   * NOT fatal. A telemetry plane that cannot be refreshed must not fail a
#     deploy that has already applied — the estate is up either way, and the
#     honest report is that the observability half is behind, said loudly.
if [ "$NETWORK" = "mainnet" ] && [ -z "$ONLY_WAVE" ] && [ -z "$HOLD" ]; then
  say ""
  say "telemetry: refreshing the scrape list from this release"
  # A pipe reports the LAST command's status, so `cmd | sed` would report sed and
  # a failed refresh would read as a clean one. Redirect, take the status, print.
  TELEMETRY_LOG="$(mktemp)"
  if bash scripts/k8s-telemetry.sh --reload >"$TELEMETRY_LOG" 2>&1; then
    sed 's/^/  /' "$TELEMETRY_LOG"
    if python3 scripts/check-prometheus-target-aliases.py --namespace "$NAMESPACE" \
         >"$TELEMETRY_LOG" 2>&1; then
      sed 's/^/  /' "$TELEMETRY_LOG"
    else
      sed 's/^/  /' "$TELEMETRY_LOG"
      say "  ^ the scrape list still has an alias collision. See micro-org#541."
    fi
  else
    sed 's/^/  /' "$TELEMETRY_LOG"
    say "  telemetry refresh FAILED. The estate is deployed; the scrape list is NOT"
    say "  current, so every SLO number below names a topology that may be older than"
    say "  this release. Run bash scripts/k8s-telemetry.sh --reload by hand."
  fi
  rm -f "$TELEMETRY_LOG"
fi
if [ -n "$HOLD" ]; then
